...
from games.myVersionOfPowerGame.gamePlayer import GamePlayer
...
from games.myVersionOfPowerGame.utilStuff import writeListToFile, \
    getFilePathWithDate
...
class PowerGame():
    ...
    def __init__(self, coordinatorId, playerList, costList, gameFreq, gameType, playerItTh):
        """
    ...
    def initPlayers(self):
        ...
        for i in range(0, len(self.nodesIdList), 2):
            self.players[i / 2] = GamePlayer(self.coordId, self.nodesIdList[i], self.nodesIdList[i + 1], self.playerCostList[i / 2], i / 2, self.gameFreq, self.gameType)
...
#                 self.dummyCrossGainsTotalInterf
    ...
    def measureGains(self):
        ...
        for p in self.players:
            ...
            self.directGains[self.players[p].getPlayerNumber()] = measureGainBetwTxRx(self.coordId, self.players[p].physicalLayer.txNode, self.players[p].physicalLayer.rxNode, self.gameFreq, txPower=0, transDuration=6)
        ...
        for p in self.players:
            ...
            for n in aux1:
                result[n.getPlayerNumber()] = measureGainBetwTxRx(self.coordId, n.physicalLayer.txNode, self.players[p].physicalLayer.rxNode, self.gameFreq, txPower=0, transDuration=6)
                # wait for things to cool down
                time.sleep(2)
    ...
    def scaleCostForPlayer(self, playerId):
        ...
        for crossGj in self.crossGains[playerId]:
            mySum += self.crossGains[playerId][crossGj] * math.pow(10.00, float(self.playersCrtTxPovers[crossGj]) / 10.00) * 1e-3
        omega = mySum / float(self.directGains[playerId])
        newCost = 1 / (float(1e-3 + omega)) + self.players[playerId].getPlayerCost() * (1 / (float(1e-6 + omega)) - 1 / (float(1e-3 + omega)))
    ...
    def writePlayerStatToFile(self, playerId, nrGameIterations, fileResults, multiRun=False, nrRun=0):
        ...
        [run number - if multiple runs],game iterations, player number, player iteration, player cost, player scaled cost, player crt discrete tx power, player crt real tx power 
        """
        playerStats = [nrGameIterations, self.players[playerId].playerNumber, self.players[playerId].playerIterations, self.players[playerId].cost, self.players[playerId].scaledCost, self.players[playerId].physicalLayer.getCrtTxPower(), self.players[playerId].physicalLayer.getBestRespUntouched()]
        if multiRun:
...
#                     self.equilibrium[key] = self.players[key].isInEquilibrium()
                ...
                if not self.isGameInEquilibrium():
                    ...
                    print "Players have reached a stable state."
                    break
        print "Game type 1 finished in %d steps (1 step = players react to another player event) and %.3f seconds." % (gameIterations, time.time() - timeStartGame)
        ...
        while True:
            ...
            if self.checkPowerEvent():
                ...
                if not self.isGameInEquilibrium():
                    ...
                    print "Players have reached a stable state."
                    break
        print "Game type 1 finished in %d steps (1 step = players react to another player event) and %.3f seconds." % (iterations, time.time() - timeStartGame)
        print "For two players 10 steps = each player makes 5 moves."
                ...
                for key in oldPlayerPowerEvent:
                    ...
                    if not oldPlayerPowerEvent[key]:
                        ...
                        if self.players[key].getNrPlayerIterations() > self.playerIterationsThreshold:
                            ...
                            if self.players[key].getNrPlayerIterations()%5 == 0 and self.players[key].getNrPlayerIterations()>4:
                                measureP = True
    ...
    def playGameType5(self):
        """Dynamic game, i.e. players come and go."""