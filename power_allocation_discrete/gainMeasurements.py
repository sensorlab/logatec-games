...
def checkConvergenceRule(nrPlayers, dictDirectGains, dictCrossGains, restrictive=False):
    ...
    if len(dictDirectGains.keys()) != len(dictCrossGains.keys()):
        print "Something is wrong, nr of direct gains and cross gains do not match. (%d != %d)" % (len(dictDirectGains.keys()), len(dictCrossGains.keys()))
        return False
    ...
    if restrictive:
        ...
        for i in dictDirectGains.keys():
            for j in dictCrossGains[i]:
                if (dictCrossGains[i][j] / float(dictDirectGains[i])) >= b:
                    print "System is not convergent: h_ji/h_ii = %.3f >= %.3f" % (dictCrossGains[i][j] / float(dictDirectGains[i]), b)
                    return False
    else:
        ...
        for i in dictDirectGains.keys():
            ...
            if (averCrossG / float(dictDirectGains[i])) >= b:
                print "System is not convergent, according to relaxed convergence rule: average(h_ji)/h_ii = %.3f >= %.3f" % (averCrossG / float(dictDirectGains[i]), b)
                return False
    ...
    if restrictive:
        ...
        for i in dictDirectGains.keys():
            if (dictCrossGains[i] / float(dictDirectGains[i])) >= b:
                print "System is not convergent: h_ji/h_ii = %.3f >= %.3f" % (dictCrossGains[i] / float(dictDirectGains[i]), b)
                return False
    else:
        # compute relaxed version of convergence rule
        b = 1 / float(nrPlayers - 1)
...
def checkConvergenceRuleForSearch(nrPlayers, listDirectGains, listCrossGains, restrictive=False):
    ...
    if len(listDirectGains) != len(listCrossGains):
        print "Something is wrong, nr of direct gains and cross gains do not match. (%d != %d)" % (len(listDirectGains), len(listCrossGains))
        return False
...
def getDirectGainsNPl(coordinatorId, nodeObjectsUsed):
    ...
    for i in range(0, len(nodeObjectsUsed), 2):
        ...
        if measuredGain is None:
            print "bad measurement between nodes %d and %d" % (nodeObjectsUsed[i].getNodeID(), nodeObjectsUsed[i + 1].getNodeID())
            return
...
#         txNode.geteratorStart(time.time(), transDuration)
    ...
    while True:
        try:
            ...
            if attempts < 2:
                attempts += 1
                print "Compute gain between %d and %d : Receive power measurement error, retry %d" % (txNode.getNodeID(), rxNodeId, attempts) 
                continue
            else:
                print "Compute gain between %d and %d : Receive power measurement error, return nothing" % (txNode.getNodeID(), rxNodeId)
    ...
    print "Gain computation between tx_%d and rx_%d: Noise power-%.6fE-12, Received power-%.6fE-12" % (txNode.getNodeID(), rxNodeId, noisePower * 1e12, receivedPower * 1e12)
    
    # compute gain
    gain = (receivedPower - noisePower) / (math.pow(10.00, txPower / 10.00) * 0.001)
    if gain < 0:
        # something is wrong, no signal was generated?
        print "Gain computation between %d and %d: Bad measurement!!! noise power is bigger than received_power, omit this measurement" % (txNode.getNodeID(), rxNodeId) 
        # wait for the signal generation stops
        sleepUntilSignalGenerationStops(startGenTime, transDuration)
        return None
    
    try:
        print "Gain between node %d and node %d: %.9fE-6  ( %.6f dB)" % (txNode.getNodeID(), rxNodeId, gain * 1e6, 10.00 * math.log10(gain))
    except Exception:
...
def measureGainBetwNTxAnd1Rx(coordinatorId, txNodes, rxNode, measuringFreq, txPower=0, transDuration=10):
    ...
    while True:
        try:
            ...
            if attempts < 2:
                attempts += 1
                print "Compute gain between %s and %d : Receive power measurement error, retry %d" % (strTxNodes, rxNodeId, attempts) 
                continue
            else:
                print "Compute gain between %s and %d : Receive power measurement error, return nothing" % (strTxNodes, rxNodeId)
                """anyway, at this point there is a signal generated, so I don't want to affect other measurements, 
                so we have to wait until signal generation stops
                """
                sleepUntilSignalGenerationStops(startGenTime, transDuration)
    ...
    receivedPower = getAveragedDataMeasurementsFromFile(coordinatorId, rxNodeId)[1][0]
    print "Gain computation between tx nodes %s and rx_%d: Noise power-%.6fE-12, Received power-%.6fE-12" % (strTxNodes, rxNodeId, noisePower * 1e12, receivedPower * 1e12)
    
    # compute gain
    gain = (receivedPower - noisePower) / (math.pow(10.00, txPower / 10.00) * 0.001)
    if gain < 0:
        # something is wrong, no signal was generated?
        print "Gain computation between tx nodes %s and %d: Bad measurement!!! noise power is bigger than received_power, omit this measurement" % (strTxNodes, rxNodeId) 
        # wait for the signal generation stops
        sleepUntilSignalGenerationStops(startGenTime, transDuration)
        return None
    
    try:
        print "Gain between tx nodes %s and node %d: %.9fE-6  ( %.6f dB)" % (strTxNodes, rxNodeId, gain * 1e6, 10.00 * math.log10(gain))
    except Exception:
        print "Exception when computing log10 from gain, gain is -%.6fE-12" % gain
